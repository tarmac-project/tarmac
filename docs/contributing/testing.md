...
Integration tests require Docker-provided services, Docker-mounted `/testdata`, or both. New service-dependent tests should use the `integration` build tag. Existing Docker-backed package tests continue to run through the Makefile integration targets.
...
- **Unit tests**: Located in `*_test.go` files without build tags and included in `make tests-unit` only when they do not require service containers
- **Integration tests**: Located in `*_integration_test.go` files with `//go:build integration` tag, or run through Docker-backed Makefile targets when they need mounted fixtures or services
